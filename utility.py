...
def process_edf(edf_file, num, SKIP_FIRST, SKIP_LAST, describe_it = False):
    print("For the file: ", edf_file)
    ...
    df = df.loc[:,['EEG Fp1-REF', 'EEG Fp2-REF', 'EEG F3-REF', 'EEG F4-REF',
    ...
    if df.isnull().to_numpy().any():
        print(f"{edf_file} contain NULL value, the function will return")
        return
    else:
        print(f"{edf_file} does not contain NULL value, the process will continue")
    
    # 7. Save to a csv file
    df.to_csv(f"{CSV_FILE_DIRC}/eeg{num}.csv",index = False) 
    print(f"The dataframe of {edf_file} have been saved to {CSV_FILE_DIRC}/eeg{num}.csv\n")
...
def get_data_n_label(data, spike_locations, duration = DURATION):
    data = data.to_numpy()
    if spike_locations != []:  # If this file contain spike
        remove_list = []
        spike_list  = []
        for sl in spike_locations:
            int_sl   = int(sl)
            decimals = sl - int(sl)
            # print(sl, int_sl, np.arange((int_sl-duration//2)*(NEW_FREQUENCY)    , (int_sl+duration//2)*NEW_FREQUENCY))
            spike_list.append(np.arange((int_sl-duration//2)*(NEW_FREQUENCY)    , (int_sl+duration//2)*NEW_FREQUENCY))
            remove_list.append(np.arange((int_sl-duration//2)*(NEW_FREQUENCY)    , (int_sl+duration//2)*NEW_FREQUENCY))
        
        # Convert to numpy array    
        spike_list  = np.concatenate(spike_list , axis=0).astype(np.int64)
        remove_list = np.concatenate(remove_list, axis=0).astype(np.int64)
        # print(spike_list.shape, spike_list)
        
        # Get the data that contain spike
        data_spike     = data[spike_list].copy()
        
        # Remove the data that contain spike from the original data
        data_non_spike = np.delete(data, remove_list, axis=0)
        
        # Concatenate spike and non-spike data
        data = np.concatenate([data_spike, data_non_spike], axis=0)
        ...
        print("Data with spike   :", data_non_spike.shape)
        print("Data without spike:", data_spike.shape)
...
def train_classifier(model, dataloader, device, num_data, optimizer, LOSS_POS_WEIGHT=torch.tensor([124.85])):
    sum_loss    = 0
    list_pred_label = []
    list_corr_label = []
...
@torch.no_grad()
def evaluate_classifier(model, dataloader, device, num_data, LOSS_POS_WEIGHT=torch.tensor([124.85])):
    sum_loss = 0
    list_pred_label = []
    list_corr_label = []
    loss_func       = nn.BCEWithLogitsLoss(pos_weight=LOSS_POS_WEIGHT.to(device))
...
def train_diffusion_classifier(model, dataloader, device, num_data, optimizer, LOSS_POS_WEIGHT=torch.tensor([124.85])):
    sum_loss    = 0
    list_pred_label = []
    list_corr_label = []
    loss_func       = nn.BCEWithLogitsLoss(pos_weight=LOSS_POS_WEIGHT.to(device))
...
@torch.no_grad()
def evaluate_diffusion_classifier(model, dataloader, device, num_data, LOSS_POS_WEIGHT=torch.tensor([124.85])):
    sum_loss = 0
    list_pred_label = []
    list_corr_label = []
    loss_func       = nn.BCEWithLogitsLoss(pos_weight=LOSS_POS_WEIGHT.to(device))
