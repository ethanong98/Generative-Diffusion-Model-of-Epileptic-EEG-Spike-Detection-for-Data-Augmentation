...
class WaveGradNN(nn.Module):
    ...
    def forward(self, x_0_dwt, x_0, noise_level, label):
        ...
        statistics = []
        # dblock_outputs = self.dblock_preconv0(x_0[:,None,:,:])      # Batch_size, 1, 19, DURATION*NEW_FREQUENCY
        # dblock_outputs = self.dblock_preconv(dblock_outputs[:,0])   # (Batch_size, 19, DURATION*NEW_FREQUENCY)
...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.cnn = nn.Sequential(
            Conv1dWithInitialization(in_channels=num_channel, out_channels=8, kernel_size=3),
            nn.BatchNorm1d(8), 
            nn.ReLU(),
            nn.MaxPool1d(4),
            Conv1dWithInitialization(in_channels=8, out_channels=16, kernel_size=3),
            nn.BatchNorm1d(16), 
            nn.ReLU(),
            nn.MaxPool1d(4),
        ...
        neuron_each_channel = (((NEW_FREQUENCY*DURATION-2)//4 -2) // 4 -2) // 4
        self.linear_layer = nn.Linear(32*neuron_each_channel,1)
...
class SateLight(nn.Module):
    ...
    def forward(self,x):
        ...
        x = x.view(x.shape[0], -1)      # Output Shape = (BATCH_SIZE, 256)
